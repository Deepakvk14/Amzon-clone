...
function Register() {
   ...
   const [lastName, setLastName]= useState();
    const register =  (e) =>{
        e.preventDefault();
        auth.createUserWithEmailAndPassword(email, password,name.lastName)
        .then((auth)=>{
                if(auth) {
                    history.push("/login")
                }
    ...
    return (
        ...
        <form>
           ...
           <button className="btn2" type="submit" onClick={register}> Create your Amazon Account</button> 
        </div>
        </div>
    )
}
